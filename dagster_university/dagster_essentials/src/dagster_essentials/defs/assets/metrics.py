...
def trips_by_week(context: dg.AssetExecutionContext, database: DuckDBResource) -> None:
    ...
    from_day = context.partition_key

    query = f"""
        select
            vendor_id, total_amount, trip_distance, passenger_count
        from trips
        where pickup_datetime >= '{from_day}'
            and pickup_datetime < '{from_day}'::date + interval '1 week'
    """
    ...
    try:
        ...
        existing = existing[existing["period"] != from_day]
